...
def difference_fft(data_img, generated_img):
    absfft2_data = np.abs(fftpack.fft2(data_img))[2:-2,2:-2]
    #absfft2_data  /= absfft2_data.max()
    absfft2_generated= np.abs(fftpack.fft2(generated_img))[2:-2,2:-2]
    #absfft2_generated /= absfft2_generated.max()
    absfft2_generated= np.abs(fftpack.fft2(generated_img))[2:-2,2:-2]
    diff_value = np.sum((absfft2_data-absfft2_generated)**2)
    return diff_value

def nl_fft(coeff, data_img,(zoomfactory,zoomfactorx)):
    """
    ...
    return difference_fft(data_img, expected)
...
def nl(coeff, data_img,(zoomfactory,zoomfactorx)):
    ...
    height = surface_polynomial(data_img.shape,coeff,(zoomfactory,zoomfactorx))
    expected= 1+ np.cos((4*np.pi/0.532)*height)
    #expected = 1+signal.sawtooth((4*np.pi/0.532)*height,0.5)
    expected /= expected.max()#normalize to 0-1float
    #expected = equalize(expected)
...
if __name__ == "__main__":
    ...
    for yy in range(0,data_img.shape[0]-dyy,dyy):
        for xx in range(0,data_img.shape[1]-dxx,dxx):#xx,yy starting upper left corner of patch
            ...
            data_patch= gaussian_filter(data_patch,sigma=1)
            ...
            while 1:
                ...
                result = basinhopping(nl, initcoeff, niter = 5, T=100, stepsize=2e-5, interval=50,accept_test=accept_test,minimizer_kwargs={'method': 'Nelder-Mead', 'args': (data_patch,(zoomfactory,zoomfactorx))}, disp=False, callback=callback)
                print result.fun
                if result.fun <560:
                    xopt = result.x
                    break
