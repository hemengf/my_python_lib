...
def accept_test(f_new,x_new,f_old,x_old):
    #return True
    if (abs(x_new[3])<0.005 and abs(x_new[4])<0.005) or (abs(x_old[3])<0.005 and abs(x_old[4])<0.005):
        return False
    else:
        return True
    ...
    if len(initcoeff_extendlist) > 0:
        ...
        for i in np.arange(alist.size):
            for j in np.arange(blist.size):
                if (j-0.5*len(blist))**2+(i)**2<=(0.5*len(alist))**2:#remove central region to avoid 0,0 global min
...
if __name__ == "__main__":
    ...
    mask_img = cv2.imread('mask_top_right.tif', 0)
    ...
    dyy,dxx =35,42 
    threshold = 0.08
    white_threshold = 0.53
    #startingposition = (60,900)
    startingposition = (278,3489)
    ...
    while any(tilequeue): 
        ...
        ax.plot_wireframe(X,Y,height,rstride=int(dyy),cstride=int(dxx))
        ax.set_aspect('equal')
        plt.draw()
        plt.pause(0.01)
        cv2.imwrite('fitimg_top_right.tif', fitimg.astype('uint8'))
        print '\n'
    np.save('xoptstore_top_right',xstore)
