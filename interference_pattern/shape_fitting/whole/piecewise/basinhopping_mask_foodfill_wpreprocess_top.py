...
###############################################################
    ...
    while 1:
        ...
        result = basinhopping(nl, initcoeff, niter = 10, T=0.001, stepsize=5e-4, interval=50,accept_test=accept_test,minimizer_kwargs={'method': 'Nelder-Mead', 'args': (data_patch,(zoomfactory,zoomfactorx), mask_patch)}, disp=False, callback=callback)
        print result.fun
...
if __name__ == "__main__":
    ...
    abquadrant = 3
    data_img = cv2.imread('sample_top.tif', 0)
    mask_img = cv2.imread('mask_top.tif', 0)
    ...
    while any(tilequeue): 
        ...
        #plt.draw()
        #plt.pause(0.01)
        cv2.imwrite('fitimg_top.tif', fitimg.astype('uint8'))
        print '\n'
    np.save('xoptstore_top'+strftime("%Y%m%d_%H_%M_%S",localtime()),xstore)
