...
def equalize(img_array):
    ...
    #equalized = exposure.equalize_adapthist(img_array,kernal_size = (2,2))
...
def difference(data_img, generated_img,mask_patch):
    ...
    """
    data_img = gaussian_filter(data_img,sigma=(0.3,0.3))
    generated_img = gaussian_filter(generated_img, sigma=(0,0))
    #plt.imshow(data_img)
    #plt.show()
    diff_value = np.sum(mask_patch*(data_img-generated_img)**2)
    diff_value /= (mask_patch.sum())
    return diff_value
...
###############################################################
    ...
    """
    # stripe equation: y = -a/b x, need to blur image in this direction
    rows, cols = data_patch.shape
    dst =  np.zeros((rows, cols))
    for tx  in range(-1,1):
        ty = -(initcoeff[3]/initcoeff[4])*tx
        M = np.float64([[1,0,tx],[0,1,ty]])
        dst += cv2.warpAffine(data_patch, M, (cols, rows))
    data_patch = dst/len(range(-1,1))
    """

    while 1:
        #print 'iternumber =', iternumber,'for',yy,xx
        result = basinhopping(nl, initcoeff, niter = 8, T=0.001, stepsize=5e-5, interval=50,accept_test=accept_test,minimizer_kwargs={'method': 'Nelder-Mead', 'args': (data_patch,(zoomfactory,zoomfactorx), mask_patch)}, disp=False, callback=callback)
        print result.fun
...
if __name__ == "__main__":
    ...
    sample_size = 0.1 #a, b value range
    abquadrant = 4
    data_img = cv2.imread('sample4.tif', 0)
    data_img = data_img.astype('float64')
    mask_img = cv2.imread('mask_left.tif', 0)
    ...
    for tx  in range(-1,2):
        ty = tx
        M = np.float64([[1,0,tx],[0,1,ty]])
        dst += cv2.warpAffine(data_img, M, (cols, rows))
    dst /= len(range(-1,2))
    data_img[:,3086:3850] = dst[:,3086:3850]
    """

    cv2.imwrite('blurred.tif', data_img)
    ...
    hstore_upperleft= {}
    dxx = 42 
    dyy =int(dxx*np.tan(np.pi*53/180)) 
    threshold = 0.084
    white_threshold = 0.3
    #startingposition = (1170,1059)
    startingposition = (1352,1388)
    ...
    while any(tilequeue): 
        ...
        ax.plot_wireframe(X,Y,height,rstride=int(dyy/4),cstride=int(dxx/4))
        ax.set_aspect('equal')
        plt.draw()
        plt.pause(0.01)
        cv2.imwrite('fitimg_left.tif', fitimg.astype('uint8'))
        print '\n'
    np.save('xoptstore_left',xstore)
