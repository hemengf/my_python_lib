...
def difference(data_img, generated_img,mask_patch):
    ...
    """
    diff_value = np.sum(((data_img-generated_img)*mask_patch)**2)
    diff_value /= (mask_patch.sum()/len(mask_patch.flat))#percentage of white area
    return diff_value
...
if __name__ == "__main__":
    ...
    mask_img = cv2.imread('masktif.tif', 0)
    ...
    goodness = np.load('goodness.npy').item()
    print goodness
    startingtile =  (2,2)
    tilequeue = find_tilequeue4([startingtile])
    print tilequeue
    tilequeue.append(startingtile)
    processed_tiles = []
    tilequeue = [tile for tile in tilequeue if tile in goodness]
    while any(tilequeue): 
        goodness_queue = {tile:goodness[tile] for tile in tilequeue}
        ...
        processed_tiles.append((int(yy/dyy),int(xx/dxx)))
        ...
        mask_patch= gaussian_filter(mask_patch,sigma=0)
        ...
        if len(initcoeff_extendlist) > 0:
        ...
        else: #if no touching tiles are detected, only for the starting tile
            if abquadrant == 1:
            ...
            for i in np.arange(alist.size):
                for j in np.arange(blist.size):
                    if (j-0.5*len(blist))**2+(i)**2<=(0.05*len(alist))**2:#remove central region to avoid 0,0 global min
        ...
        while 1:
            ...
            result = basinhopping(nl, initcoeff, niter = 5, T=100, stepsize=2e-5, interval=50,accept_test=accept_test,minimizer_kwargs={'method': 'Nelder-Mead', 'args': (data_patch,(zoomfactory,zoomfactorx), mask_patch)}, disp=False, callback=callback)
            print result.fun
        ...
        tilequeue = find_tilequeue4(processed_tiles)
        tilequeue = [tile for tile in tilequeue if tile in goodness]
        print '\n'
